...
#  define LDAV_DONE
    ...
    if ((kn = kstat_data_lookup(ksp, "avenrun_1min")) == 0) {
...
#  endif
    ...
    if ((fd = open(LINUX_LDAV_FILE, O_RDONLY | O_NOFOLLOW)) == -1) {
        return -1;
    }
    count = read(fd, ldavgbuf, (size_t) 40U);
    (void) close(fd);
    if (count <= (ssize_t) 0) {
        return -1;
    }
...
#  endif
    ...
    if ((fp = fopen(NETBSD_LDAV_FILE, "r")) == NULL) {
        return -1;
    }
