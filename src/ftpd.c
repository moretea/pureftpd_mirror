...
/* Convert a 4-in-6 address in pure IPv4 */
        ...
        *++hd = 0;
        ...
        if (create_home_and_chdir(authresult.dir)) {
            /* non-chrooted users can see everything, so let them
             in anyway */
            if (userchroot == 0) {
                addreply(0, MSG_NO_HOMEDIR2 " [%s].",
                         authresult.dir, strerror(errno));
                addreply_noformat(0, MSG_START_SLASH);
                chdir("/");
            } else {
                die(421, LOG_ERR, MSG_NO_HOMEDIR);
            }
...
# endif
    ...
    doreply();
    state_needs_update = 1;
    setprogname("pure-ftpd (DOWNLOAD)");
...
# endif
    ...
    hints.ai_family = v6ready ? AF_UNSPEC : AF_INET;
    hints.ai_socktype = SOCK_STREAM;
    hints.ai_addr = NULL;
    if ((on = getaddrinfo(standalone_ip, standalone_port, &hints, &res)) != 0 ||
        (res->ai_family != AF_INET && res->ai_family != AF_INET6)) {
        logfile(LOG_ERR, MSG_SYNTAX_ERROR_IP ": [%s]", strerror(errno));
...
# endif
        ...
        case 'f': {
            ...
            if (facilitynames[n].c_name) {
                ...
                logfile(LOG_ERR, MSG_CONF_ERR ": " MSG_ILLEGAL_FACILITY ": %s", optarg);
            }
            break;
        }
        case 'l': {
    ...
    if (first_authentications == NULL) {
        if ((first_authentications = malloc(sizeof *first_authentications)) == NULL) {
            die_mem();
        }
